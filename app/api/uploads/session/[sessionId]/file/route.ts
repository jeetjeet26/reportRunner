...
export async function POST(req: NextRequest, { params }: { params: { sessionId: string } }) {
  ...
  if (Date.now() > m.expiresAt) return new Response(JSON.stringify({ error: "expired" }), { status: 410, headers: { "content-type": "application/json" } });

  const form = await req.formData().catch(() => null);
  if (!form) return new Response(JSON.stringify({ error: "form_required" }), { status: 400, headers: { "content-type": "application/json" } });
  const file = form.get("file");
  if (!(file instanceof File)) return new Response(JSON.stringify({ error: "file_required" }), { status: 400, headers: { "content-type": "application/json" } });
  const maxMb = 25;
  if (file.size > maxMb * 1024 * 1024) return new Response(JSON.stringify({ error: `max_${maxMb}mb` }), { status: 400, headers: { "content-type": "application/json" } });
  const ct = (file as any).type || "";
  const name = (file as any).name || "upload.pdf";
  const lower = (ct || "").toLowerCase();
  if (!lower.includes("pdf") && !name.toLowerCase().endsWith(".pdf")) {
    return new Response(JSON.stringify({ error: "non_pdf" }), { status: 400, headers: { "content-type": "application/json" } });
  }
  ...
  return new Response(JSON.stringify({ uploadedCount: m.uploadedCount, canSelect }), { status: 200, headers: { "content-type": "application/json" } });
}
