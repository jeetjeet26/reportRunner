...
export function gateChannelsInMarkdown(markdown: string, allowedChannels: string[]): string {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (line.match(/^###\s+/)) {
      ...
      // Determine if this header looks like a channel header at all
      const allKeywords = Object.values(channelKeywords).flat();
      const isChannelLike = allKeywords.some(keyword => headerText.includes(keyword));

      // Check if this header corresponds to an allowed channel
      ...
      const shouldSkipChannelSection = isChannelLike && !isAllowedChannel;
      skipUntilNextSection = shouldSkipChannelSection;

      if (!shouldSkipChannelSection) {
        result.push(line);
      }
    } 
